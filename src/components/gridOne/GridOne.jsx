...
export default function GridOne(props) {
    ...
    useEffect(() => {
        ...
        let leftBound = Math.min(inp[0][1], inp[1][1]);
        let rightBound = Math.max(inp[0][1], inp[1][1]);
        let topBound = Math.min(inp[0][0], inp[1][0]);
        let bottomBound = Math.max(inp[0][0], inp[1][0]);
